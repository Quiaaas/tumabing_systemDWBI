...
import { ArrowLeft, ArrowRight, Check, CheckCircle2, FileUp, Loader2, LockKeyhole, LogOut, Save, Trash2, UploadCloud } from "lucide-react";
      ...
      <div className="mx-auto max-w-[1180px] px-5 py-6 sm:px-8 lg:px-10">
        <header className="flex items-center justify-between border-b border-[#dce2dc] pb-6">
          <button className="flex items-center gap-3 text-left" onClick={() => navigate("/dashboard")} type="button">
            <span className="grid size-10 place-items-center rounded-full bg-[#102b3a] text-[#f0c36b]"><LockKeyhole className="size-5" /></span>
            <span><strong className="block text-sm text-[#102b3a]">Student Admissions</strong><small className="text-xs uppercase tracking-[0.18em] text-[#7b8889]">Admission application</small></span>
          </button>
        ...
        <section className="py-9 lg:py-12">
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div><p className="mb-3 text-xs font-bold uppercase tracking-[0.22em] text-[#a26b18]">2026–2027 admissions</p><h1 className="font-serif text-4xl tracking-[-0.04em] text-[#102b3a] sm:text-5xl">Build your application.</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-[#65747a]">Take your time. Your progress is saved in this browser between steps.</p></div>
            <div className="flex items-center gap-2 text-xs text-[#718087]"><Save className="size-4 text-[#a26b18]" /> Draft saved automatically</div>
          </div>
          ...
          <form onSubmit={submitApplication} className="mt-6">
            <div className="rounded-2xl border border-[#dce2dc] bg-white p-5 shadow-[0_12px_30px_rgba(21,35,45,0.04)] sm:p-8">
              ...
              <div className="mt-8 flex flex-col-reverse justify-between gap-3 border-t border-[#edf0eb] pt-6 sm:flex-row">
                ...
                {step < 5 ? <Button type="button" onClick={next} className="gap-2 rounded-xl bg-[#102b3a] text-white hover:bg-[#1d4558]">Continue <ArrowRight className="size-4" /></Button> : <Button type="submit" disabled={submit.isPending} className="gap-2 rounded-xl bg-[#102b3a] text-white hover:bg-[#1d4558]">{submit.isPending ? <Loader2 className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />}{submit.isPending ? "Submitting…" : "Submit application"}</Button>}
              </div>
            </div>
          </form>
        </section>
...
function StepFive({ draft, campuses, programs, documentRules }: { draft: ApplicationDraft; campuses: Array<{ campus_id: number; campus_name: string | null }>; programs: Array<{ program_id: number; program_name: string | null; college: string | null }>; documentRules: ReturnType<typeof getDocumentRules> }) {
  ...
  return <StepShell eyebrow="Step 5 of 5" title="Review & submit" description="Review your information carefully. Submit only when everything looks correct.">
    ...
    <div className="mt-6 flex items-start gap-3 rounded-xl border border-[#dce2dc] bg-[#f6f7f2] p-4 text-sm leading-6 text-[#65747a]"><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-[#28604f]" />Submitting will save your applicant details, create an admission application with status <strong className="text-[#102b3a]">Pending</strong>, and record your uploaded document metadata.</div>
  </StepShell>;
}

function StepShell({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children: ReactNode }) {
  return <div><p className="text-xs font-bold uppercase tracking-[0.22em] text-[#a26b18]">{eyebrow}</p><h2 className="mt-3 font-serif text-3xl tracking-[-0.03em] text-[#102b3a] sm:text-4xl">{title}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#65747a]">{description}</p><div className="mt-8 space-y-6">{children}</div></div>;
}

function Field({ label, required, children }: { label: string; required?: boolean; children: ReactNode }) {
  return <div className="space-y-2"><Label>{label}{required && <span className="ml-1 text-[#a26b18]">*</span>}</Label>{children}</div>;
}
...
function ApplicationSkeleton() {
  return <main className="min-h-screen bg-[#f6f7f2] p-6"><div className="mx-auto max-w-[1180px] animate-pulse space-y-8"><div className="h-12 rounded-xl bg-[#e4e9e3]" /><div className="h-24 rounded-2xl bg-[#e4e9e3]" /><div className="h-[620px] rounded-2xl bg-white" /></div></main>;
}
