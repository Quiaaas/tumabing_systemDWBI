...
export default function Enrollment() {
  ...
  return <main className="min-h-screen bg-[#f7fbf8] text-[#15232d]"><div className="mx-auto max-w-[1180px] px-5 py-6 sm:px-8 lg:px-10">
    <header className="flex items-center justify-between border-b border-[#dce2dc] pb-6"><button className="text-left" onClick={() => navigate("/dashboard")} type="button"><BrandMark /></button><div className="flex items-center gap-4"><span className="hidden text-xs font-semibold uppercase tracking-[0.15em] text-[#087f5b] sm:inline">{CHMSU_BRAND.tagline}</span><Button variant="ghost" onClick={() => navigate("/dashboard")} className="gap-2 text-[#65747a]"><LogOut className="size-4" /> Exit</Button></div></header>
    <section className="py-9 lg:py-12"><div><p className="mb-3 text-xs font-bold uppercase tracking-[0.22em] text-[#087f5b]">Approved applicant</p><h1 className="font-serif text-4xl tracking-[-0.04em] text-[#07563f] sm:text-5xl">Complete your enrollment.</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-[#65747a]">Your admission is approved. Finish the details below to create your enrollment record.</p></div>
      <nav className="mt-9 grid grid-cols-3 gap-1 rounded-2xl border border-[#dce2dc] bg-white p-2 sm:grid-cols-6 sm:gap-2 sm:p-3" aria-label="Enrollment progress">{steps.map((label, index) => <button type="button" key={label} onClick={() => index + 1 < step && setStep(index + 1)} className={`min-w-0 rounded-xl px-2 py-2 text-left text-xs font-semibold ${step === index + 1 ? "bg-[#07563f] text-white" : step > index + 1 ? "bg-[#effaf5] text-[#28604f]" : "text-[#8b9798]"}`}><span className="mr-1.5 inline-grid size-5 place-items-center rounded-full border border-current text-[10px]">{step > index + 1 ? <Check className="size-3" /> : index + 1}</span><span className="hidden sm:inline">{label}</span></button>)}</nav>
      <form onSubmit={submitEnrollment} className="mt-6"><div className="rounded-2xl border border-[#dce2dc] bg-white p-5 shadow-[0_12px_30px_rgba(21,35,45,0.04)] sm:p-8">
        ...
        <div className="mt-8 flex flex-col-reverse justify-between gap-3 border-t border-[#edf0eb] pt-6 sm:flex-row"><Button type="button" variant="outline" onClick={back} disabled={step === 1} className="gap-2 rounded-xl border-[#d5ded8]"><ArrowLeft className="size-4" /> Back</Button>{step < 6 ? <Button type="button" onClick={next} className="gap-2 rounded-xl bg-[#07563f] text-white hover:bg-[#1d4558]">Continue <ArrowRight className="size-4" /></Button> : <Button type="submit" disabled={submit.isPending} className="gap-2 rounded-xl bg-[#07563f] text-white hover:bg-[#1d4558]">{submit.isPending ? <Loader2 className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />}{submit.isPending ? "Submitting…" : "Submit enrollment"}</Button>}</div>
      </div></form>
    </section></div></main>;
}
...
function Shell({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children: ReactNode }) { return <div><p className="text-xs font-bold uppercase tracking-[0.22em] text-[#087f5b]">{eyebrow}</p><h2 className="mt-3 font-serif text-3xl tracking-[-0.03em] text-[#07563f] sm:text-4xl">{title}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#65747a]">{description}</p><div className="mt-8 space-y-6">{children}</div></div>; }
function Field({ label, required, children }: { label: string; required?: boolean; children: ReactNode }) { return <div className="space-y-2"><Label>{label}{required && <span className="ml-1 text-[#087f5b]">*</span>}</Label>{children}</div>; }
function ReviewSection({ title, rows }: { title: string; rows: Array<[string, string]> }) { return <section className="rounded-2xl border border-[#dce2dc] bg-[#fbfcf9] p-5"><h3 className="text-sm font-bold uppercase tracking-[0.14em] text-[#087f5b]">{title}</h3><dl className="mt-4 space-y-3">{rows.length ? rows.map(([label, value]) => <div className="grid grid-cols-[minmax(96px,0.45fr)_1fr] gap-3 text-sm" key={label}><dt className="text-[#8b9798]">{label}</dt><dd className="break-words font-medium text-[#07563f]">{value || "—"}</dd></div>) : <p className="text-sm text-[#8b9798]">None</p>}</dl></section>; }
function EnrollmentSkeleton() { return <main className="min-h-screen bg-[#f7fbf8] p-6"><div className="mx-auto max-w-[1180px] animate-pulse space-y-8"><div className="h-12 rounded-xl bg-[#e4e9e3]" /><div className="h-24 rounded-2xl bg-[#e4e9e3]" /><div className="h-[620px] rounded-2xl bg-white" /></div></main>; }
